...
fetchData <- function(i) {
    ...
    if (tempTradingDay < '20151001') {
      ...
      tempLine <- c(grep('合约',tempData),length(tempData)+1)
      ...
      res <- lapply(2:length(tempLine),function(kk){
          ...
          ## 去掉首尾行
          tempdt <- tempdt[-1]
          if (any(grepl('合计', tempdt))) tempdt <- tempdt[-grep('合计', tempdt)]
      ...
      res <- lapply(seq(1,length(grep('合约|合计', tempData$X1)), by = 2),function(kk){
        ...
        ## 去掉首尾行
        tempdt <- tempdt[-grep('合约|名次|合计|总|计',X1)]
...
fetchData <- function(i) {
    ...
    if (any(grepl("商品名称|会员|期货公司|注",tempData$X1))) {
      tempData <- tempData[-grep("商品名称|会员|期货公司|注",X1)]
    }

    tempLine <- c(grep('合约代码', tempData$X1),nrow(tempData)+1)

    ## -------------------------------------------------------------------------
    res <- lapply(2:length(tempLine), function(kk){
        tempTable <- tempData[tempLine[kk-1] : (tempLine[kk]-1)]
        ...
        ## 去掉首尾行
        tempTable <- tempTable[-grep('合约|代码|名次|加总|合计|注',X1)]
    ...
    ## -------------------------------------------------------------------------
    res <- res[!is.na(Rank)][!is.na(Amount)]
    if (any(grepl('会员|公司', res$Rank))) res <- res[-grep('会员|公司', Rank)]
    return(res)
}
