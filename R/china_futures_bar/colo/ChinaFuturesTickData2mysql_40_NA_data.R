...
if(nrow(myDay) > 20000 | grep('history', the_script_main)){##-- 全天的
  break_time_detector <- data.table(BreakBeginTime = c("21:00:00","00:00:00","09:00:00","13:00:00"),
    ...
    .[, DataFile := ifelse(c(data_file_1,data_file_2,data_file_3)  %>% .[!is.na(.)]  %>% length() != 0,
  ...
  if( as.numeric(format(Sys.time(), "%H")) %between% c(8, 20) ){##---- 日盘 ----------------##
    break_time_detector <- data.table(BreakBeginTime = c("09:00:00","13:00:00"),
                                      BreakEndTime   = c("11:30:00","15:15:00")
    )
  }else{##------------------------------------------------------------ 夜盘 ----------------##
    break_time_detector <- data.table(BreakBeginTime = c("21:00:00","00:00:00"),
                                      BreakEndTime   = c("23:59:59","02:30:00")
    )
  }
...
break_time_detector[,':='(
  ...
  ,DataSource  = paste(args_input[1],args_input[2], sep = "_")
)]
...
log_info$RscriptSub  <- paste('(1) ChinaFuturesTickData2mysql_10_read_data.R',
                              ...
                              '            : (3) ChinaFuturesTickData2mysql_40_NA_data.R',
                              sep = " \n ")

log_info$results     <- paste(info$status, collapse = "\n ")
################################################################################~~~~~~~~~~~~~~~~~~~~~~~~~~
mysql <- dbConnect(MySQL(), dbname = "dev", host="127.0.0.1",                 ## dev
                   user = mysql_user, password = mysql_pw)
