...
info <- dbGetQuery(mysql, "
  select * from info_YunYang1_FromPC
  where TradingDay = 20171130
  ") %>% as.data.table() %>% 
...
dtRes[errTurnover != 0][Turnover.x != 0][abs(Turnover.x - Turnover.y) / Turnover.y > 0.1]
dtRes[, ":="(vwap = Turnover.x / Volume.x / VolumeMultiple,
             vwap2 = Turnover.y / Volume.y / VolumeMultiple)]
dtRes[abs(vwap -vwap2)/vwap2 > 0.005][Turnover.x != 0]
## =============================================================================
