...
## -----------------------------------------------------------------------------
dbSendQuery(mysql,paste0("DELETE FROM minute_options_",coloSource,
            " WHERE TradingDay = ", logTradingDay))
dbWriteTable(mysql,paste0("minute_options_",coloSource),
             dtMinute %>%
...
dbSendQuery(mysql,paste0("DELETE FROM daily_",coloSource,
            " WHERE TradingDay = ", logTradingDay))
## -----------------------------------------------------------------------------
dbSendQuery(mysql,paste0("DELETE FROM daily_options_",coloSource,
            " WHERE TradingDay = ", logTradingDay))
...
if ( tempHour %between% c(15,20) | includeHistory ) {
  ...
  dbWriteTable(mysql, paste0("daily_",coloSource),
               ...
               , row.name　=　FALSE, append = T)
  ## -- DailyBar
  dbWriteTable(mysql, paste0("daily_options_",coloSource),
               rbind(dt_allday, dt_day, fill = TRUE) %>%
               .[grep("-P-|-C-|[0-9]{2,3}P[0-9]{2,3}|[0-9]{2,3}C[0-9]{2,3}", InstrumentID)]
               , row.name　=　FALSE, append = T)
}
...
if (nrow(dt_night) != 0) {  #--- 如果非空，则录入 MySQL 数据库
  dbWriteTable(mysql, paste0("daily_",coloSource),
               dt_night %>% .[nchar(InstrumentID) < 8] %>%
                 .[grep('[0-9]',InstrumentID)], row.name　=　FALSE, append = T)
...
## -----------------------------------------------------------------------------
dbSendQuery(mysql,paste0("DELETE FROM tick_options_",coloSource,
            " WHERE TradingDay = ", logTradingDay))
dbWriteTable(mysql, paste0("tick_options_",coloSource),
             dtTick %>%
             .[grep("-P-|-C-|[0-9]{2,3}P[0-9]{2,3}|[0-9]{2,3}C[0-9]{2,3}", InstrumentID)]
             , row.name = FALSE, append = T)
}
