...
SAVE_PATH <- "/home/fl/myData/data/MarketIndex/stocks_index"
## =============================================================================
...
fetchIndex <- function(url, exchID) {
  ...
  colnames(dt) <- c('exchID','stockID','stockName',
                    'lastClose', 'open', 'high', 'low', 
                    'lastPrice','volume', 'turnover',
                    'chg','pct','pa','pm')
  dt[, exchID := gsub('[0-9]','',exchID)]
  dt[, ":="(lastClose = NULL, chg = NULL, pct = NULL,
            pa = NULL, pm = NULL)]
  return(dt)
}
...
dt[, exchID := NULL]
setnames(dt, 'lastPrice', 'close')
dt[, TradingDay := currTradingDay[1, days]]
dt <- dt[, .(TradingDay, stockID, stockName, 
             open, high, low, close, 
             volume, turnover)]
destFile <- paste0(SAVE_PATH, '/fromJRJ/', 
                   currTradingDay[1, gsub('-','',days)], '.csv')
fwrite(dt, destFile)

mysql <- mysqlFetch('MarketIndex')
dbSendQuery(mysql, paste("delete from stocks_index where TradingDay = ", 
                        currTradingDay[1, gsub('-','',days)]))
dbWriteTable(mysql, 'stocks_index', dt, row.names = F, append = T)
dbDisconnect(mysql)
