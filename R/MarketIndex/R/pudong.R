...
if (format(Sys.Date(), '%Y-%m-%d') != currTradingDay[1,days]) stpp('NOT TradingDay!!!')
...
resTable[grep(':', 时间), 时间 := as.character(as.Date(currTradingDay[1,days],'%Y%m%d'))]
resTable[grep('/', 时间), 时间 := as.character(as.Date(lastTradingDay[1,days],'%Y%m%d'))]

## =============================================================================
fwrite(resTable, paste0(DATA_PATH,'/',
                        as.character(currTradingDay[1,days]),
                        '_NanhuaIndex.csv'))
remDr$close()
## =============================================================================
