  ...
  tempRes <- temp %>%
    .[,.SD[,.(
      OpenPrice = ifelse(nrow(.SD[DeltaVolume != 0]) != 0,
                        ifelse(all(.SD[Volume != 0, is.na(OpenPrice)]) |
                               all(.SD[Volume != 0, OpenPrice == 0]) |
                               daySector == 'day',
                              .SD[DeltaVolume != 0][1, LastPrice], .SD[Volume != 0][!is.na(OpenPrice)][OpenPrice != 0][1,OpenPrice]),
                        ifelse(all(.SD[Volume != 0, is.na(OpenPrice)]) |
                               all(.SD[Volume != 0, OpenPrice == 0]) |
                               daySector == 'day',
                              .SD[Volume != 0][1, LastPrice], .SD[Volume != 0][!is.na(OpenPrice)][OpenPrice != 0][1,OpenPrice])),
      HighPrice = ifelse(all(is.na(.SD$HighestPrice)) | sum(.SD$HighestPrice, na.rm=TRUE) == 0 | daySector == 'day',
                         max(.SD[Volume != 0]$LastPrice, na.rm=TRUE),
                         max(.SD[Volume != 0]$HighestPrice, na.rm=TRUE)),
      LowPrice  = ifelse(all(is.na(.SD$LowestPrice)) | sum(.SD$LowestPrice, na.rm=TRUE) == 0 | daySector == 'day',
      ...
      ClosePrice = ifelse(all(is.na(.SD$ClosePrice)) | sum(.SD$ClosePrice, na.rm=TRUE) == 0 |
                          ...
                          .SD[Volume != 0][LowestPrice != 0][.N,LastPrice],
                          .SD[Volume != 0][LowestPrice != 0][.N,ClosePrice]),
      #-----------------------------------------------------------------------------
    ...
    .[, Sector := daySector]
  # if (nrow(tempRes) != 0) {
  #   if (substr(tempRes[,unique(TradingDay)],1,4) != '2010')
  #     tempRes <- tempRes[Turnover != 0]
  # }
  #-----------------------------------------------------------------------------
  #>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
