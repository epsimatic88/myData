...
## 历史的交易日历由上证指数来推算
## 可以从 网易财经 下载上证指数
## Ref: http://quotes.money.163.com/service/chddata.html?code=0000001&start=19901219&end=20180112&fields=TCLOSE;HIGH;LOW;TOPEN;LCLOSE;CHG;PCHG;VOTURNOVER;VATURNOVER
## 
temp <- read_csv('./data/ChinaFinancialCalendar/sh000001.csv',
                 locale = locale(encoding = "GB18030")) %>% 
        as.data.table() %>% 
        .[, .(days = as.character(日期))] %>% 
        .[, days := gsub('-', '', days)] %>%
        .[days < '20180101']
calendar_sh <- list(temp, calendar2018[, .(days)]) %>% 
        rbindlist() %>% 
        .[order(days)]

fwrite(calendar_sh, './data/ChinaFinancialCalendar/ChinaStocksCalendar.csv')
dbSendQuery(mysql, "
            CREATE TABLE IF NOT EXISTS dev.ChinaStocksCalendar(
              days Date NOT NULL,
              PRIMARY KEY (days)
            )
            ")
dbSendQuery(mysql,"truncate table ChinaStocksCalendar")
dbWriteTable(mysql, "ChinaStocksCalendar",
             calendar_sh, row.names=FALSE, append=TRUE)
dbDisconnect(mysql)
