...
CREATE DATABASE `china_futures_bar` DEFAULT CHARACTER SET utf8 COLLATE utf8_general_ci;
...
## china_futures_bar.daily                                                    ## china_futures_bar.daily
################################################################################～～～～～～～～～～～～
CREATE TABLE  china_futures_bar.daily_options(
    TradingDay       DATE             NOT NULL,            ## 交易日期
    Sector           CHAR(20)         NOT NULL,            ## 日期属性: 
    #                                                      ## 1. 只含日盘: Sector = 'day'
    #                                                      ## 2. 只含夜盘: Sector = 'nights'
...
ON china_futures_bar.daily_options
(TradingDay, Sector, InstrumentID);  
## -------------------------------------------------------------------------- ## 

##----------- PARTITIONS ---------------------------------------------------- ##
ALTER TABLE china_futures_bar.daily_options
    PARTITION BY RANGE( TO_DAYS(TradingDay) )(
    #---------------------------------------------------------------------------
    PARTITION p_2011_01 VALUES LESS THAN (TO_DAYS('2017-01-01')),
    PARTITION p_2017_01 VALUES LESS THAN (TO_DAYS('2017-02-01')),
    PARTITION p_2017_02 VALUES LESS THAN (TO_DAYS('2017-03-01')),
    PARTITION p_2017_03 VALUES LESS THAN (TO_DAYS('2017-04-01')),
    PARTITION p_2017_04 VALUES LESS THAN (TO_DAYS('2017-05-01')),
...
CREATE TABLE  china_futures_bar.minute_options(
    TradingDay       DATE           NOT NULL,              ## 交易日期
    Minute           TIME           NOT NULL,              ## 分钟，格式为==> "HH:MM:SS"", 与 Wind 数据库类似
    NumericExchTime  DECIMAL(15,5)  NOT NULL,              ## 分钟的数值格式，以 18:00::00 为正负界限，
    #                                                      ## 注意：取的是有 tick 的第一个，不一定是这个分钟开始的值
...
ON china_futures_bar.minute_options
(TradingDay, Minute, InstrumentID);  
## -------------------------------------------------------------------------- ## 

##----------- PARTITIONS ---------------------------------------------------- ##
ALTER TABLE china_futures_bar.minute_options
    PARTITION BY RANGE( TO_DAYS(TradingDay) )(
    #---------------------------------------------------------------------------
    PARTITION p_2011_01 VALUES LESS THAN (TO_DAYS('2017-01-01')),
    PARTITION p_2017_01 VALUES LESS THAN (TO_DAYS('2017-02-01')),
...
################################################################################～～～～～～～～～～～～～
## china_futures_bar.main                                                     ## china_futures_bar.main
################################################################################～～～～～～～～～～～～～

CREATE TABLE  china_futures_bar.main(
    TradingDay       DATE           NOT NULL,              ## 交易日期
    Product          CHAR(30)       NOT NULL,              ## 品种
    main             CHAR(30)       NOT NULL,              ## 主力合约
    prodIndex        decimal(8,4)   ,                      ## 指数
    #-----------------------------------------------------
    PRIMARY KEY (TradingDay, Product, main)         ## 主键唯一，重复不可输入
)


CREATE TABLE  china_futures_bar.multiplier(
    TradingDay       DATE           NOT NULL,              ## 交易日期
    InstrumentID     CHAR(30)       NOT NULL,              ## 品种
    multiplier       BIGINT         , 
    #-----------------------------------------------------
