...
BROKER = 'DeBang'
ACCOUNTS = ['QJ6']
LOCAL_BASE_DIR = 'c:\\logs\\autohook\\stock\\debang'
EXPORT_BASE_DIR = '\\\\192.168.1.66\\public\\hotdata\\stock\\debang'
...
class DeBangTDX(TDX):
    def __init__(self, broker, account, local_base_dir, export_base_dir):
        super(DeBangTDX, self).__init__(broker, account, local_base_dir, export_base_dir)
        ...
        self.cid_sell_entrust = '[CLASS:Button; INSTANCE:46]'
        self.cid_sell_able = '[CLASS:Static; INSTANCE:104]'
        self.cid_cancel_refresh = '[CLASS:Button; INSTANCE:64]'
        self.cid_cancel_selectall = '[CLASS:Button; INSTANCE:66]'
        self.cid_cancel_do = '[CLASS:Button; INSTANCE:63]'
        ...
        self.cid_entrust_refresh = '[CLASS:Button; INSTANCE:70]'
        self.cid_entrust_export = '[CLASS:Button; INSTANCE:76]'
        self.cid_trade_refresh = '[CLASS:Button; INSTANCE:116]'
        self.cid_trade_export = '[CLASS:Button; INSTANCE:122]'

#==================================================================

class DeBangInterface(DeBangTDX, Interface):
    def __init__(self, account):
        if account not in ACCOUNTS:
            raise InitError('account %s is not registered' % (account))
        super(DeBangInterface, self).__init__(BROKER, account, LOCAL_BASE_DIR, EXPORT_BASE_DIR)
...
class DeBangService(DeBangTDX):
    def __init__(self, account):
        if account not in ACCOUNTS:
            raise InitError('account %s is not registered' % (account))
        super(DeBangService, self).__init__(BROKER, account, LOCAL_BASE_DIR, EXPORT_BASE_DIR)
        ...
        try:
            for line in src_file:
                if len(line) < 50 or line.find('-------') == 0 or line.find(u'委托时间') == 0 or \
                    line.find(u'撤买') != -1 or line.find(u'撤卖') != -1 or line.find(u'没有相应的查询信息!') != -1:
                    continue
                else:
                    out = {}
                    tokens = list(filter(lambda x : x != '', map(unicode.strip, line.strip().split('     '))))
